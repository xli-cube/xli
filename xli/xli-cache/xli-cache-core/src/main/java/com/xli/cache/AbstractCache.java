...
public abstract class AbstractCache<K, V> implements Cache<K, V> {
    ...
    public static <K, V> V synchronizedLoad(CacheConfig config, AbstractCache<K, V> abstractCache, K key, Function<K, V> newLoader, Consumer<V> cacheUpdater) {
    ...
    private static <K, V> boolean needUpdate(V loadedValue, boolean cacheNullWhenLoaderReturnNull, Function<K, V> loader) {
        ...
        return !(loader instanceof CacheLoader) || !((CacheLoader<K, V>) loader).vetoCacheUpdate();
    }

    private static Object buildLoaderLockKey(Cache c, Object key) {
        switch (c) {
            case AbstractEmbeddedCache abstractEmbeddedCache -> {
                return abstractEmbeddedCache.buildKey(key);
            }
            case AbstractExternalCache abstractExternalCache -> {
                byte[] bytes = abstractExternalCache.buildKey(key);
                return ByteBuffer.wrap(bytes);
            }
            case MultiLevelCache multiLevelCache -> {
                c = multiLevelCache.caches()[0];
                return buildLoaderLockKey(c, key);
            }
            case ProxyCache proxyCache -> {
                c = proxyCache.getTargetCache();
                return buildLoaderLockKey(c, key);
            }
            case null, default -> throw new CacheException("impossible");
        }
    }

    public static <K, V> V computeIfAbsentImpl(K key, Function<K, V> loader, boolean cacheNullWhenLoaderReturnNull, long expireAfterWrite, TimeUnit timeUnit, Cache<K, V> cache) {
    ...
    public final V computeIfAbsent(K key, Function<K, V> loader, boolean cacheNullWhenLoaderReturnNull, long expireAfterWrite, TimeUnit timeUnit) {
        return computeIfAbsentImpl(key, loader, cacheNullWhenLoaderReturnNull, expireAfterWrite, timeUnit, this);
    }
    ...
    protected void logError(String oper, Object key, Throwable e) {
        ...
        sb.append("xli.cache(").append(this.getClass().getSimpleName()).append(") ").append(oper).append(" error.");
